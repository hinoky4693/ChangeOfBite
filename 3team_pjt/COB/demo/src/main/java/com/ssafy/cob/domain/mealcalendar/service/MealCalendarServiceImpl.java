...
public class MealCalendarServiceImpl implements MealCalendarService {
	...
	public boolean registMeal(List<MealCalendar> mealCalendar, int userId) {
		for(MealCalendar meal : mealCalendar) {
			meal.setUserId(userId);
			if(mealCalendarDao.insertMeal(meal) == 0) return false;
		}
		return true;
	}
	...
	public boolean modifyMeal(MealCalendar mealCalendar, List<MultipartFile> files) {
		if(mealCalendarDao.updateMeal(mealCalendar) <= 0) return false; 
		fileDao.deleteFilesByRelated("CALENDAR", mealCalendar.getId());
		saveFiles("CALENDAR", mealCalendar.getId(), files);
		return true;
	}
}
